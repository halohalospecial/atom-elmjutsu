...
export default class InferTypes {
    ...
    if (helper.isElmEditor(editor)) {
      ...
      helper.doInTempEditor((tempEditor) => {
        ...
        if (isNothingSelected) {
          ...
          const lineLength = tempEditor.getBuffer().lineLengthForRow(selectedRange.start.row);
          ...
          if (endsWithArrow || endsWithEqual) {
            ...
            if (endsWithEqual && lineTokens.length === 1) {
              tempEditor.setCursorBufferPosition([selectedRange.start.row, selectedRange.start.column]);
              tempEditor.insertText(selectedText + ' : ' + dummyType() + '\n' + leadingSpaces);
            } else {
              ...
              tempEditor.setCursorBufferPosition([selectedRange.start.row, lineLength + dummyVariable().length]);
              tempEditor.insertText('\n' + leadingSpaces + ' let _ = (' + dummyFunction() + ' <| ' + selectedText + ') in\n');
            }
          } else {
            tempEditor.setTextInBufferRange(selectedRange, '(' + dummyFunction() + ' <| ' + selectedText + ')');
          }
        }
  ...
  doInferTypes(code, inferenceType, inferenceName, editor, shouldDecorate, shouldDestroyOnChange, shouldShowAutocomplete, getInferenceFunction) {
    const self = this;
    ...
    atomLinter.tempFile(path.basename(inputFilePath), code, (tempFilePath) => {
      ...
      .then(data => {
        ...
        const numErrors = elmMakeHelper.getErrorsAndWarnings(problems)[0].length;
        const inferences = numErrors === 0 ? [defaultInference] :
          _.flatten(problems)
          .map((problem) => {
        ...
        if (inference) {
          ...
          if (shouldShowAutocomplete) {
            const onAutocompleteActivate = helper.forceActivateAutocomplete(editor);
