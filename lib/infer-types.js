...
export default class InferTypes {
          ...
          ({ match, matchText, range, stop, replace }) => {
            if (match && match.length > 3) {
              ...
              if (originalModuleName) {
                replace(matchText.replace(originalModuleName, dummyModule()));
              }
              stop();
            }
          }
  ...
  doInferTypes(
    ...
    const usingLegacyVersion = isLegacyElmVersion(elmVersion);
    const executablePath = usingLegacyVersion
    ...
    atomLinter.tempFile(path.basename(inputFilePath), code, tempFilePath => {
      ...
      return atomLinter
        ...
        .then(data => {
          ...
          const problems = usingLegacyVersion
            ? elmMakeHelper.parseProblemsLegacy(data, projectDirectory)
          ...
          const numErrors = usingLegacyVersion
...
function getInferredTypeFromProblem(problem) {
  if (isLegacyElmVersion(problem.elmVersion)) {
    return getInferredTypeFromProblemLegacy(problem);
  }
...
function getInferredTypeFromProblemLegacy(problem) {
  let regex;
  let matches;
...
function isLegacyElmVersion(elmVersion) {
