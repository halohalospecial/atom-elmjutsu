...
export default class InferTypes {
  ...
  constructor(indexer) {
    ...
    let inferExpressionTypeDebouncer = null;
    this.subscriptions.add(atom.textEditors.observe((editor) => {
      if (helper.isElmEditor(editor)) {
        ...
        this.subscriptions.add(editor.onDidChangeSelectionRange(({selection}) => {
          ...
          if (atom.config.get('elmjutsu.inferExpressionTypeOnTheFly')) {
            this.destroyMarkers('expression type');
            if (inferExpressionTypeDebouncer) {
              clearTimeout(inferExpressionTypeDebouncer);
              inferExpressionTypeDebouncer = null;
            }
            inferExpressionTypeDebouncer =
              setTimeout(() => {
                this.inferExpressionTypeCommand();
            }, 200);
          }
        }));
  ...
  inferHoleTypesCommand() {
    ...
    if (helper.isElmEditor(editor)) {
      ...
      if (origCode !== modifiedCode) {
        this.inferQueue.add(() => {
          return this.inferTypes(inputCode, 'hole types', helper.holeToken(), editor, true, (problem, editor) => {
            return {
  ...
  inferExpressionTypeCommand() {
    ...
    if (helper.isElmEditor(editor)) {
      ...
      // Abort if nothing is selected and cursor is not between spaces (excluding indents).
      if (isNothingSelected && (!/\s$/.test(prefix) || (isAllSpaces && prefix.length > 1 && charAfter !== '') || !/^(\s|\n|)$/.test(charAfter))) {
        return;
      }
      // Abort if nothing is selected and the token is not blank.
      const token = helper.getToken(editor);
      if (isNothingSelected && token && token.trim().length > 0) {
        return;
      }
      let code;
      editor.transact(() => {
      ...
      this.inferQueue.add(() => {
        return this.inferTypes(code, 'expression type', '↓', editor, false, (problem, editor) => {
          return {
            type: getInferredTypeFromProblem(problem),
            range: selectedRange
          };
  ...
  inferTypes(code, inferenceType, inferenceName, editor, shouldDecorate, getInferenceFunction) {
  ...
  getInferenceAtCursorPosition(editor) {
    const markers =
      ...
      .filter((marker) => { return marker.getProperties().inference; });
    if (markers.length > 0) {
