            ...
            setTimeout(() => {
              if (!self.activeTokenHints || self.activeTokenHints.length === 0) {
                ...
                const markedStrings = _.flatten(self.activeTokenHints.map((hint) => {
                  ...
                  if (atom.config.get('elmjutsu.showSourcePathsInSidekick') && hint.sourcePath) {
                    const packageDocsPrefix = helper.getPackageDocsPrefix();
                    if (hint.sourcePath.startsWith(packageDocsPrefix)) {
                      const sourcePathDisplay = hint.sourcePath.replace(packageDocsPrefix, '');
                      markdownItems.push('[' + sourcePathDisplay + '](' + hint.sourcePath + ' "' + hint.sourcePath + '")');
                    } else if (self.activeFile) {
                      const sourcePathDisplay = hint.sourcePath.replace(self.activeFile.projectDirectory + path.sep, '');
                      markdownItems.push(sourcePathDisplay);
                    }
                  }
