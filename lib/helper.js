...
export default {
  ...
  getTokenRange(position, editor) {
    let scanRange = [[position.row, 0], position];
    let beginningOfWordPosition = position;
    editor.backwardsScanInBufferRange(this.tokenRegex(), scanRange, ({range, matchText, stop}) => {
    ...
    editor.scanInBufferRange(this.tokenRegex(), scanRange, ({range, matchText, stop}) => {
      if (matchText !== '' || range.start.column !== 0) {
  ...
  getActiveTopLevel(editor, position) {
    if (this.isElmEditor(editor)) {
      ...
      let block = null;
      let matchRange = null;
      if (position.column === 0 && !/^(\s|)$/.test(editor.getTextInBufferRange([position, position.translate([0,1])]))) {
        editor.scanInBufferRange(this.blockRegex(), [position, editor.getEofBufferPosition()], ({match, range, stop}) => {
          stop();
        ...
        editor.backwardsScanInBufferRange(this.blockRegex(), [[0, 0], position], ({match, range, stop}) => {
          stop();
          if (!this.moduleOrImportBlockRegex().test(match[3])) {
            block = match[3];
            matchRange = range;
      ...
      if (block && matchRange) {
        let prefix = '';
        const prefixes = _.sortBy(['type alias ', 'type ', 'port '], (s) => { return -s.length; });
        ...
        editor.scanInBufferRange(/(\S+)\s/, [matchRange.start.translate([0, prefix.length]), editor.getEofBufferPosition()], ({match, stop}) => {
          stop();
  ...
  scanForSymbolDefinitionRange(editor, symbol, func) {
    ...
    let numJumps = nameParts.length === 2 && nameParts[0] === nameParts[1] ? 2 : 1;
    if (topLevel) {
      sourcePathParts.pop();
      sourcePathParts.pop();
