...
export default {
  ...
  getToken(editor) {
    ...
    if (this.isTokenAString(scopeDescriptor)) {
      return '';
    }
  ...
  allImportsRegex() {
    // TODO: Make this work even if there are newlines between import statements.
  ...
  getTypeAnnotationAbove(editor) {
    ...
    editor.backwardsScanInBufferRange(this.blockRegex(), [[0, 0], editor.getCursorBufferPosition()], ({match, matchText, stop}) => {
      stop();
      const block = match[3];
      if (!this.moduleOrImportBlockRegex().test(block) &&
          !this.typeAliasBlockRegex().test(block) &&
