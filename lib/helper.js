...
export default {
  ...
  getTypeAnnotationAbove(editor) {
    ...
    editor.backwardsScanInBufferRange(this.blockRegex(), [[0, 0], [editor.getCursorBufferPosition().row, 0]], ({match, stop}) => {
      stop();
      const block = match[3];
      if (!this.moduleOrImportBlockRegex().test(block) &&
          !this.typeAliasBlockRegex().test(block) &&
  ...
  scanForSymbolDefinitionRange(editor, symbol, func) {
    ...
    if (topLevel) {
      ...
      symbolDefinitionRegex = new RegExp(recordRegex + prefixedRegex + functionArgRegex, 'm');
    } else {
      ...
      symbolDefinitionRegex = new RegExp(tipeCaseRegex + prefixedRegex + moduleRegex + functionRegex, 'm');
    }
