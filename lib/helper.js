...
export default {
  ...
  scanForSymbolDefinitionRange(editor, symbolName, caseOf, func) {
    const prefixedRegex = '^(?:(?:port module|module|import|type alias|type|port)\\s+)(' + _.escapeRegExp(symbolName) + ')(?:\\s|$)|';
    const typeCaseRegex = caseOf ? '^(?:type\\s+' + _.escapeRegExp(caseOf) + '(?:.|\\n)*?(?:(?:=|\\|)\\s+))(' + _.escapeRegExp(symbolName) + ')(?:\\s|$)|' : '';
    const defaultRegex = '^(\\(?' + _.escapeRegExp(symbolName) + '\\)?)(?:\\s(?!:.*))';
    const symbolDefinitionRegex = new RegExp(typeCaseRegex + prefixedRegex + defaultRegex, 'm');
