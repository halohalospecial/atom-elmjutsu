...
export default class Indexer {
  ...
  watchProject(indexer, editor) {
    ...
    if (atom.inDevMode()) {
      console.log('[elm-fu] Parsing source files of `' + projectDirectory + '`...');
    }
    ...
    if (atom.inDevMode()) {
      console.log('[elm-fu] Parsed source files of `' + projectDirectory + '`');
    }
    ...
    watcher.on('change', (filename) => {
      ...
      if (atom.inDevMode()) {
        console.log('[elm-fu] `change` detected - ' + filePath);
      }
    ...
    watcher.on('add', (filename) => {
      const filePath = path.join(projectDirectory, filename);
      if (atom.inDevMode()) {
        console.log('[elm-fu] `add` detected - ' + filePath);
      }
      if (filename === 'elm-package.json') {
        sendNewPackages(projectDirectory, indexer);
      } else {
    ...
    watcher.on('unlink', (filename) => {
      const filePath = path.join(projectDirectory, filename);
      if (atom.inDevMode()) {
        console.log('[elm-fu] `unlink` detected - ' + filePath);
      }
      if (filename === 'elm-package.json') {
  ...
  filenames.forEach((filename) => {
    ...
    if (filePath !== exceptForFilePath) {
      ...
      if (stats.isDirectory() && filename !== 'elm-stuff') {
        ...
        if (atom.inDevMode()) {
          console.log('[elm-fu] Parsing ' + filePath);
        }
        const text = fs.readFileSync(filePath, {encoding: 'utf8'});
        setTimeout(() => {
          indexer.ports.sourceFileChangedSub.send([filePath, parseModuleDocs(text, filePath), parseImports(text)]);
