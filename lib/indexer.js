...
export default class Indexer {
  ...
  findUsages() {
    const editor = atom.workspace.getActiveTextEditor();
    if (helper.isElmEditor(editor)) {
      this.indexer.ports.getImportersForTokenSub.send([helper.getProjectDirectory(editor.getPath()), helper.getToken(editor)]);
    }
  }
  ...
  watchProject(indexer, editor) {
    ...
    devLog('[elmjutsu] Parsing source files of `' + projectDirectory + '`...');
    ...
    devLog('[elmjutsu] Parsed source files of `' + projectDirectory + '`');
    ...
    watcher.on('change', (filename) => {
      ...
      devLog('[elmjutsu] `change` detected - ' + filePath);
    ...
    watcher.on('add', (filename) => {
      const filePath = path.join(projectDirectory, filename);
      devLog('[elmjutsu] `add` detected - ' + filePath);
      if (filename === 'elm-package.json') {
        sendNewPackages(projectDirectory, indexer);
      } else {
        sendFileContentsChanged(filePath, indexer);
    ...
    watcher.on('unlink', (filename) => {
      const filePath = path.join(projectDirectory, filename);
      devLog('[elmjutsu] `unlink` detected - ' + filePath);
      if (filename === 'elm-package.json') {
  ...
  filenames.forEach((filename) => {
    ...
    // if (filePath !== exceptForFilePath) {
      ...
      if (stats.isDirectory() && filename !== 'elm-stuff') {
        ...
        devLog('[elmjutsu] Parsing ' + filePath);
        const text = fs.readFileSync(filePath).toString();
        setTimeout(() => {
          indexer.ports.fileContentsChangedSub.send([filePath, parseModuleDocs(text, filePath), parseImports(text)]);
        });
