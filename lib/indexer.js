...
export default class Indexer {
  ...
  constructor() {
    ...
    indexer.ports.goToDefinitionCmd.subscribe((uri) => {
      ...
      atom.workspace.open(filePath, {searchAllPanes: true, split: 'left'})
        .then((editor) => {
          editor.scanInBufferRange(helper.symbolDefinitionRegex(valueName), [[0, 0], editor.getEofBufferPosition()], ({match, range, stop}) => {
  ...
  findSymbol() {
    const editor = atom.workspace.getActiveTextEditor();
    if (helper.isElmEditor(editor)) {
      this.indexer.ports.findSymbolSub.send([helper.getProjectDirectory(editor.getPath()), getToken(editor)]);
    }
  }
...
function parseModuleDocs(text, filePath) {
  ...
  if (match && match.length > 4 && match[4]) {
    ...
    const {aliases, types, values} = parseModuleValues(text);
    return {
...
function parseModuleValues(text) {
  ...
  while (match) {
    ...
    if (block.startsWith('import ')) {
      // Ignore.
    } else if (block.startsWith('module ')) {
      // Ignore.
    } else if (block.startsWith('port module ')) {
      // Ignore.

    } else if (block.startsWith('type alias ')) {
      const parts = block.replace('type alias ', '').split('=');
      ...
      const parts = block.replace('type ', '').split('=');
    ...
    } else if (block.startsWith('port ')) {
      const parts = block.replace('port ', '').split(':');
