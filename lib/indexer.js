...
export default class Indexer {
  ...
  returnFromDefinition() {
    const activeEditor = atom.workspace.getActiveTextEditor();
    const activeEditorPath = activeEditor.getPath();
    const jumpPoint = this.jumpPoints[activeEditorPath];
    if (jumpPoint) {
      atom.workspace.open(jumpPoint.uri, {searchAllPanes: true, split: 'left'})
        .then((editor) => {
          editor.setCursorBufferPosition(jumpPoint.position);
          editor.scrollToCursorPosition({center: true});
          delete this.jumpPoints[activeEditorPath];
        });
    }
  }
  ...
  watchProject(indexer, editor) {
    ...
    if (atom.inDevMode()) {
      console.log('[elmjutsu] Parsing source files of `' + projectDirectory + '`...');
    }
    ...
    if (atom.inDevMode()) {
      console.log('[elmjutsu] Parsed source files of `' + projectDirectory + '`');
    }
    ...
    watcher.on('change', (filename) => {
      ...
      if (atom.inDevMode()) {
        console.log('[elmjutsu] `change` detected - ' + filePath);
      }
      if (filename !== 'elm-package.json') {
        ...
        sendSourceFileChanged(filePath, indexer);
      }
    });
    watcher.on('add', (filename) => {
      const filePath = path.join(projectDirectory, filename);
      if (atom.inDevMode()) {
        console.log('[elmjutsu] `add` detected - ' + filePath);
      }
      if (filename === 'elm-package.json') {
        sendNewPackages(projectDirectory, indexer);
      } else {
    ...
    watcher.on('unlink', (filename) => {
      const filePath = path.join(projectDirectory, filename);
      if (atom.inDevMode()) {
        console.log('[elmjutsu] `unlink` detected - ' + filePath);
      }
      if (filename === 'elm-package.json') {
...
function sendSourceFileChanged(filePath, indexer) {
  ...
  filenames.forEach((filename) => {
    ...
    if (filePath !== exceptForFilePath) {
      ...
      if (stats.isDirectory() && filename !== 'elm-stuff') {
        ...
        if (atom.inDevMode()) {
          console.log('[elmjutsu] Parsing ' + filePath);
        }
        const text = fs.readFileSync(filePath, {encoding: 'utf8'});
        setTimeout(() => {
          indexer.ports.sourceFileChangedSub.send([filePath, parseModuleDocs(text, filePath), parseImports(text)]);
