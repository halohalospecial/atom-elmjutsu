...
export default class AddImportView extends ModalListView {
  ...
  viewForItem({moduleName, symbolName}) {
    if (symbolName) {
      ...
      const symbolNameClass = symbolName[0] === symbolName[0].toUpperCase() ? 'syntax--storage' : 'syntax--entity';
      const symbolNameClause = symbolName === '..' ? `<span class="syntax--keyword">(..)</span>`: `<span class="syntax--punctuation">(</span><span class="${symbolNameClass}">${symbolName}</span><span class="syntax--punctuation">)</span>`;
