...
export default class FindUsages {
  ...
  constructor(indexer, core) {
    ...
    indexer.ports.importersForTokenReceivedCmd.subscribe(([projectDirectory, token, isCursorAtLastPartOfToken, sourcePathAndNamesList]) => {
      var allLineUsages = [];
      sourcePathAndNamesList.forEach(([sourcePath, willUseFullToken, names]) => {
        ...
        text.split('\n').forEach((line, row) => {
          const lineUsages = parseLine(sourcePath, row, line, token, isCursorAtLastPartOfToken, willUseFullToken, names);
...
function parseLine(sourcePath, row, rawLine, token, isCursorAtLastPartOfToken, willUseFullToken, names) {
