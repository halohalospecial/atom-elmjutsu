  ...
  return {
    ...
    getSuggestions({editor, bufferPosition}) {
      return new Promise(resolve => {
        ...
        if (typeAnnotation) {
          const getFunctionHeadFromTypeAnnotationPromise = new Promise((resolve) => {
            return getFunctionHeadFromTypeAnnotation(prefix, typeAnnotation, indexer, resolve);
          });
          return Promise.all([
            getFunctionHeadFromTypeAnnotationPromise,
    ...
    onDidInsertSuggestion({editor, triggerPosition, suggestion}) {
      ...
      if (suggestion.constructedFromTypeAnnotation && editor.lineTextForBufferRow(editor.getCursorBufferPosition().row).trim() !== '') {
...
function getFunctionHeadFromTypeAnnotation(prefix, typeAnnotation, indexer, resolve) {
  const functionHeadFromTypeAnnotationReceived = (functionHead) => {
    indexer.ports.functionHeadFromTypeAnnotationReceivedCmd.unsubscribe(functionHeadFromTypeAnnotationReceived);
    if (functionHead.startsWith(prefix)) {
      let suggestion = {
          replacementPrefix: prefix,
          text: functionHead,
          rightLabelHTML: '<i>* construct from type annotation *</i>',
          constructedFromTypeAnnotation: true // Custom field.
        };
      // if (atom.config.get('elmjutsu.autocompleteSnippetsEnabled')) {
      //   const parts = functionHead.replace(/ =\n    $/m, '').split(' ');
      //   if (parts.length > 1) {
      ...
      //     suggestion.displayText = functionHead;
      //   }
      // }
  ...
  indexer.ports.functionHeadFromTypeAnnotationReceivedCmd.subscribe(functionHeadFromTypeAnnotationReceived);
  indexer.ports.getFunctionHeadFromTypeAnnotationSub.send(typeAnnotation);
}
