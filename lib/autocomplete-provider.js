...
export function provide(indexer, shouldGetSuggestionsFunction, getInferenceAtCursorPositionFunction, clearCursorInferenceFunction, setAutocompleteActiveFunction) {
  ...
  setTimeout(() => {
    ...
    subscriptions.add(atom.config.observe('elmjutsu.autocompleteEnabled', enabled => {
      if (enabled && atom.config.get('language-elm.autocompleteEnabled')) {
        const notification = atom.notifications.addInfo('It\'s recommended to disable `language-elm`\'s autocomplete feature', {
          description: 'Since `elmjutsu`\'s autocomplete feature is enabled, it\'s recommended to disable the autocomplete feature of `language-elm`.<br><br>' +
    ...
    if (autocompletePackage) {
      const autocompleteManager = autocompletePackage.mainModule.autocompleteManager || autocompletePackage.mainModule.getAutocompleteManager();
      ...
      // This hack does not do anything for Atom v1.19.1 and above.
...
function updateDescription(suggestion, entry) {
  ...
  entry.descriptionMoreURL = suggestion.sourcePath === '' ? null : suggestion.sourcePath;
  return entry;
}
...
function getHintsForPartial(partial, suffix, inferredType, preceedingToken, succeedingToken, isRegex, isTypeSignature, indexer, editor, triggerPosition, resolve) {
  ...
  const autocompleteFuzzyFilteringEnabled = atom.config.get('elmjutsu.autocompleteFuzzyFilteringEnabled');
  const hintsForPartialReceived = ([partial, hints]) => {
    ...
    if (autocompleteFuzzyFilteringEnabled && !isRegex && !isTypeSignature && (!inferredType || (inferredType && partial !== ''))) {
    ...
    const suggestions =
      ...
      .map((hint) => {
        ...
        let suggestion = updateDescription(hint, {
          replacementPrefix: isRegex || isTypeSignature ? partial.slice(0, partial.length-suffix.length) : partial,
          text: text,
  ...
  indexer.ports.getHintsForPartialSub.send([partial, inferredType, preceedingToken, isRegex, isTypeSignature, isFiltered, isGlobal]);
}
