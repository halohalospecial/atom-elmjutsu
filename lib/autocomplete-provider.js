    ...
    getSuggestions({editor, bufferPosition}) {
      ...
      return new Promise(resolve => {
        ...
        const getHintsForPartialPromise = new Promise((resolve) => {
          let preceedingToken, succeedingToken;
          ...
          return getHintsForPartial(activeToken, inferredType, shouldRemoveLastArgument(preceedingToken, succeedingToken), indexer, editor, bufferPosition, resolve);
        });
...
function getHintsForPartial(partial, inferredType, shouldRemoveLastArgument, indexer, editor, triggerPosition, resolve) {
  ...
  const hintsForPartialReceived = ([partial, hints]) => {
    ...
    const suggestions =
      ...
      .map((hint) => {
        ...
        if (atom.config.get('elmjutsu.autocompleteSnippetsEnabled')) {
          suggestion.snippet = hintToSnippet(hint, shouldRemoveLastArgument);
          if (hint.isImported) {
    ...
    // helper.log('getHintsForPartial, partial: ' + partial + ', inferredType: ' + inferredType + ', suggestions: ' + suggestions.length + ', time: ' + (performance.now() - timeStart) + ' ms');
...
function constructDefaultArguments(token, shouldRemoveLastArgument, indexer, resolve) {
