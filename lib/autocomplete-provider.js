      ...
      subscriptions.add(
        atom.commands.onWillDispatch(event => {
          ...
          if (
            ...
            helper.isElmEditor(event.target.getModel())
          ) {
      ...
      return new Promise(resolve => {
        ...
        const getHintsForPartialPromise = new Promise(resolve => {
          ...
          const partial = isTypeSignature
            ...
            : isRegex ? regexMatch[1] : activeToken;
        ...
        // Ending with space:
        const endingWithSpace = prefix.match(/(?:\b)(\S+)\s$/) ? true : false;
        if (endingWithSpace) {
...
function getSuggestionsForImport(prefix, partial, indexer, resolve) {
  ...
  indexer.postMessage({
    type: 'getSuggestionsForImport',
    data: [partial, isFiltered],
  });
}
...
function constructFromTypeAnnotation(prefix, typeAnnotation, indexer, resolve) {
  ...
  indexer.postMessage({
    type: 'constructFromTypeAnnotation',
    data: typeAnnotation,
  });
}
...
function constructDefaultArguments(
  ...
  indexer.postMessage({
    type: 'constructDefaultArguments',
    data: token,
  });
}
...
function constructDefaultValueForType(
  ...
  indexer.postMessage({
    type: 'constructDefaultValueForType',
    data: token,
  });
}
