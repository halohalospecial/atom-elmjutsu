      ...
      return new Promise(resolve => {
        ...
        if (match) {
          const constructLetInPromise = new Promise((resolve) => {
            return resolve([{
              ...
              snippet: getLeadingSpaces(prefix) +
                'let\n' +
                helper.tabSpaces() + '${1:} =\n' +
                helper.tabSpaces() + helper.tabSpaces() + '${2:}\n' +
                'in\n' +
                helper.tabSpaces() + '${3:}',
              // leftLabelHTML: specialCompletionIcon(),
              rightLabelHTML: specialCompletionIcon() + '<i>Write let/in</i>',
              displayText: 'let in'
            }]);
          });
          return Promise.all([
          ...
          const constructIfThenPromise = new Promise((resolve) => {
            return resolve([{
              ...
              snippet: getLeadingSpaces(match[0]) +
                ...
                helper.tabSpaces() + '${3:}',
              // leftLabelHTML: specialCompletionIcon(),
              rightLabelHTML: specialCompletionIcon() + '<i>Write if/then/else</i>',
              displayText: 'if then else'
            }]);
          });
          return Promise.all([
        ...
        if (match) {
          return resolve([{
            ...
            // leftLabelHTML: specialCompletionIcon(),
            rightLabelHTML: specialCompletionIcon() + '<i>Write module</i>'
          }]);
        }
...
function constructCaseOf(prefix, token, indexer, resolve) {
  const caseOfConstructed = (construction) => {
    ...
    if (construction) {
      const text = getLeadingSpaces(prefix) + 'case ' + token + ' of\n' +
        construction
        // Insert tab stops.
        .split('|')
        .map((part, i) => {
      ...
      let suggestion = {
          ...
          snippet: text,
          // leftLabelHTML: specialCompletionIcon(),
          rightLabelHTML: specialCompletionIcon() + '<i>Write case/of</i>'
        };
      return resolve([suggestion]);
    }
    return resolve([]);
