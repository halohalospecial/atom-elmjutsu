  ...
  return {
    ...
    getSuggestions({ editor, bufferPosition }) {
      ...
      return new Promise(resolve => {
        ...
        // Construct Html program skeleton:
        if (atom.config.get('elmjutsu.insertHtmlProgramCompletionEnabled')) {
          match = editor
            .getText()
            .match(
              /^\s*(program|Html\.program|html\.program|Htmlprogram|htmlprogram)\s*$/
            );
          if (match) {
            return resolve([
              {
