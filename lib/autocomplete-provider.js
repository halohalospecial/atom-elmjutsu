  ...
  return {
    ...
    getSuggestions({editor, bufferPosition}) {
      return new Promise(resolve => {
        ...
        // e.g. `    case stringMaybe`
        match = prefix.match(/\s+case\s(.+)\s$/);
        if (match) {
...
function constructCaseOf(prefix, tipe, indexer, resolve) {
  ...
  indexer.ports.constructCaseOfSub.send(tipe);
}
