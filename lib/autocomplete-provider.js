    ...
    getSuggestions({editor, bufferPosition}) {
      ...
      return new Promise(resolve => {
        ...
        const getHintsForPartialPromise = new Promise((resolve) => {
          return getHintsForPartial(helper.getToken(editor), indexer, editor, resolve);
        });
...
function getHintsForPartial(partial, indexer, editor, resolve) {
...
function hintToSnippet(hint) {
  ...
  return hint.name + hint.args.reduce((acc, arg, i) => {
    return acc + ' ${' + (i + 1) + ':' + arg + '}';
  }, '');
}
