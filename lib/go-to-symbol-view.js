...
export default class GoToSymbolView extends SelectListView {
  ...
  viewForItem({fullName, kind, caseTipe, sourcePath}) {
    ...
    // return `<li>${iconSpan}${moduleNameSpan}<span class="symbol-kind-${dashedSymbolKind}">${symbolName}</span>${kindSpan}</li>`;
    return `<li><span class="symbol-kind-${dashedSymbolKind}">${symbolName}</span>${kindSpan}</li>`;
  }
