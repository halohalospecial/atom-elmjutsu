...
export default {
  ...
  sendActiveTextAndToken(indexer, editor) {
    ...
    indexer.ports.clearLocalHintsCacheSub.send(null);
    indexer.ports.fileContentsChangedSub.send([filePath, projectDirectory, this.parseModuleDocs(text, filePath), this.parseImports(text)]);
  ...
  sendTokenAtRange(indexer, editor, range) {
    const token = editor.getTextInBufferRange(range);
    indexer.ports.activeTokenChangedSub.send([helper.getActiveTopLevel(editor, range.start), token]);
  },
