...
export default {
  ...
  sendFileContentsChanged(indexer, filePath) {
    ...
    indexer.postMessage({
      type: 'fileContentsChanged',
      data: [filePath, projectDirectory, this.parseModuleDocs(text, filePath), this.parseImports(text)]
    });
  },

  sendFileContentsRemoved(indexer, filePath, projectDirectory) {
    indexer.postMessage({
      type: 'fileContentsRemoved',
      data: [filePath, projectDirectory]
    });
  },
  ...
  sendActiveFile(indexer, editor) {
    ...
    indexer.postMessage({
      ...
      data: [{filePath, projectDirectory}, helper.getActiveTopLevel(editor), helper.getToken(editor)]
    });
  },
  ...
  sendActiveTextAndToken(indexer, editor) {
    ...
    indexer.postMessage({
      type: 'activeTokenChanged',
      data: [helper.getActiveTopLevel(editor), helper.getToken(editor)]
    });
  },
  ...
  sendToken(indexer, editor, range, token) {
    indexer.postMessage({
      type: 'activeTokenChanged',
      data: [helper.getActiveTopLevel(editor, range.start), token]
    });
  },

  sendEnteredInference(indexer, inference) {
    indexer.postMessage({
      type: 'inferenceEntered',
      data: inference
    });
  },
  ...
  sendProjectDependencies(indexer, projectDirectory, workDirectory) {
    ...
    if (updatedDependencies) {
      indexer.postMessage({
        type: 'projectDependenciesChanged',
        data: [projectDirectory, updatedDependencies]
      });
    }
  },
