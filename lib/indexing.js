...
export default {
  ...
  sendNewPackages(indexer, projectDirectory) {
    ...
    // Not using `elm-stuff/exact-dependencies.json` because that requires that the project was already compiled.
    const packages = _.pairs(elmPackageJson.dependencies).map(([packageName, versionRange]) => {
      return [packageName, versionRange.split(' ')[0]];
    });
    indexer.ports.projectDependenciesChangedSub.send([projectDirectory, packages]);
  },
