...
export default {
  ...
  sendFileContentsChanged(indexer, filePath) {
    ...
    indexer.ports.fileContentsChangedSub.send([filePath, this.parseModuleDocs(text, filePath), this.parseImports(text)]);
  },
  ...
  parseModuleDocs(text, filePath) {
    var match = this.parseModuleNameAndComment(text);
    if (match) {
  ...
  parseModuleValues(text) {
    ...
    while (match) {
      ...
      if (/^(port module|module|import)\s/.test(block)) {
        ...
        const valueName = formatSymbolName(match[4]);
