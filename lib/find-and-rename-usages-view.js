...
export default class FindAndRenameUsagesView extends ScrollView {
  constructor() {
    ...
    this.renameEditorView.classList.add(
      'atom-text-editor',
  ...
  getCheckedUsages(callback) {
    const checkedUsagesReceived = usages => {
      this.elm.ports.checkedUsagesReceivedCmd.unsubscribe(
        checkedUsagesReceived
      );
      callback(usages);
    };
  ...
  setContents(
    ...
    this.elm.ports.setContentsSub.send([
      projectDirectory + path.sep,
      token,
      selectedIndex,
      usagesWithChecked,
      willShowRenamePanel,
  ...
  viewInEditor(usage) {
    return atom.workspace
      .open(usage.sourcePath, { split: 'left', pending: true })
      .then(editor => {
