...
export default class FindAndRenameUsagesView extends ScrollView {
  ...
  getIncludedUsages(callback) {
    const includedUsagesReceived = (usages) => {
      this.elm.ports.includedUsagesReceivedCmd.unsubscribe(includedUsagesReceived);
      callback(usages);
    };
    this.elm.ports.includedUsagesReceivedCmd.subscribe(includedUsagesReceived);
    this.elm.ports.getIncludedUsagesSub.send(null);
  }
