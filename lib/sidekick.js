...
export default class Sidekick {
  constructor(indexer, storeJumpPointFunction) {
    ...
    this.subscriptions.add(
      atom.config.observe('elmjutsu.showSidekick', showSidekick => {
        if (showSidekick) {
          this.show(
            atom.config.get('elmjutsu.sidekickPosition'),
            atom.config.get('elmjutsu.sidekickSize')
          );
        } else {
          this.hide();
        }
    ...
    this.subscriptions.add(
      atom.config.observe('elmjutsu.sidekickPosition', sidekickPosition => {
    ...
    this.subscriptions.add(
      atom.config.observe('elmjutsu.sidekickSize', sidekickSize => {
        if (atom.config.get('elmjutsu.showSidekick')) {
          this.updateSize(
            sidekickSize,
            atom.config.get('elmjutsu.sidekickPosition')
    ...
    this.subscriptions.add(
      atom.workspace.observeActivePaneItem(item => {
        if (
          item &&
          (helper.isElmEditor(item) ||
            (item.getURI && item.getURI() === helper.usagesViewURI()))
        ) {
  ...
  show(sidekickPosition, sidekickSize) {
    const options = {
      ...
      visible: true,
      priority:
        Number.MAX_SAFE_INTEGER * (sidekickPosition === 'bottom' ? -1 : 1),
    };
...
function getConfig() {
  return {
    ...
    showDocComments:
      atom.config.get('elmjutsu.showDocCommentsInSidekick') || false,
    showAssociativities:
      atom.config.get('elmjutsu.showAssociativitiesInSidekick') || false,
    showPrecedences:
      atom.config.get('elmjutsu.showPrecedencesInSidekick') || false,
    showAliasesOfType:
      atom.config.get('elmjutsu.showAliasesOfTypesInSidekick') || false,
    showSourcePaths:
      atom.config.get('elmjutsu.showSourcePathsInSidekick') || false,
  };
}
