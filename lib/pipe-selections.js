  ...
  applyToEditor(code) {
    ...
    if (!helper.fileExists(preludePath)) {
      return atom.notifications.addError(
        'Could not find `Eval Prelude` file.',
        {
          dismissable: true,
        }
      );
    ...
    const proc = this.executeInRepl(
      ...
      (errString, outString) => {
        ...
        if (errString.length > 0) {
          ...
          const outputs = _.isArray(result)
            ? result.map(chuva => {
                return chuva.toString();
              })
            : [result.toString()];
          editor.transact(() => {
            let newRanges = [];
            ...
            _.zip(outputs, oldSelections).forEach(([output, selection]) => {
              if (selection) {
                ...
                const newSelection = editor.addSelectionForBufferRange([
                  newestRangeEnd,
                  newestRangeEnd,
                ]);
            ...
            if (newRanges.length > 0) {
              ...
              editor.setCursorBufferPosition(
                oldSelections[0].getBufferRange().start
              );
            }
    ...
    try {
      ...
      return atom.notifications.addError(
        'Could not read `Eval Prelude` file.',
        {
          dismissable: true,
        }
      );
    ...
    const imports = helper.defaultImports(elmVersion);
    // If it's a single selection, get the first element.
    const maybeConcat =
      oldSelections.length === 1
    ...
    editor.getSelections().forEach(selection => {
      ...
      this.selectionMarkers.push(
        helper.markRange(
          ...
          'elmjutsu-pipe-selection-start'
        )
      );
      this.selectionMarkers.push(
        helper.markRange(
          editor,
          [end.traverse([0, -1]), end],
          'elmjutsu-pipe-selection-end'
        )
      );
    });
  }
