...
export default class PipeSelections {
  ...
  applyToEditor(code) {
    ...
    // If it's a single selection, add a `String.concat`.
    const maybeConcat = oldSelections.length === 1 ? ' |> String.concat' : '';
    this.sendToRepl(proc, imports + '\n' + prelude + '\n');
    this.sendToRepl(proc, JSON.stringify(selectedTexts) + maybeConcat + ' |> ' + code);
  }
