...
export default class FindUsages {
  ...
  constructor(indexer, core) {
    ...
    this.subscriptions.add(atom.workspace.addOpener((uriToOpen) => {
      if (uriToOpen === getURI()) {
        this.view = new FindAndRenameUsagesView(indexer);
        return this.view;
      }
    }));
  }
  ...
  createView(state) {
    return new FindAndRenameUsagesView(state);
  }
      ...
      .then((view) => {
        if (isFindAndRenameUsagesView(view)) {
          ...
          setTimeout(() => {
            ...
            view.setUsages(projectDirectory, tokenToHighlight, usages, willShowRenamePanel);
          }, 0);
        }
      });
  }
  ...
  doRenameSymbol() {
    ...
    const {oldName, projectDirectory, usages} = this.renameInfo;
