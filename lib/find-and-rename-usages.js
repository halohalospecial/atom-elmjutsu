...
export default class FindUsages {
  constructor(indexer, storeJumpPointFunction) {
    ...
    this.subscriptions.add(
      atom.commands.add('atom-text-editor.elmjutsu-rename-symbol', {
        'elmjutsu:cancel-rename-symbol': this.hideCommand.bind(this), // escape
        'elmjutsu:confirm-rename-symbol': this.confirmRenameSymbolCommand.bind(
          this
        ), // enter
      })
    );
  ...
  show(
    ...
    atom.workspace
      .open(helper.usagesViewURI(), { searchAllPanes: true, split: 'right' })
      .then(view => {
  ...
  doFindUsages(indexDelta, callback) {
    ...
    if (helper.isElmEditor(editor)) {
      usageFinder.findUsagesOfSymbolAtCursor(
        this.indexer,
        editor,
        indexDelta || 0,
        (projectDirectory, tokenToHighlight, selectedIndex, usages) => {
        ...
        (projectDirectory, tokenToHighlight, selectedIndex, usages) => {
          if (usages.length > 0) {
            ...
            atom.notifications.addError(
              "Sorry, I don't know how to rename that."
            );
          }
        }
      );
    ...
    this.view.getCheckedUsages(usages => {
      ...
      _.pairs(groupsBySourcePath).forEach(([sourcePath, usages]) => {
        ...
        _.pairs(groupsByRow).forEach(([row, usages]) => {
          ...
          while (i < usagesSorted.length) {
            if (i === usagesSorted.length - 1) {
              ...
              slices.push(
                lineText.slice(
                  usagesSorted[i].range.end.column,
                  usagesSorted[i + 1].range.start.column
                )
              );
            }
            i = i + 1;
        ...
        if (editor) {
          ...
          indexing.sendActiveEditorContents(this.indexer, editor);
