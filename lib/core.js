...
export default class Core {
  ...
  constructor() {
    ...
    this.indexer.ports.downloadDocsFailedCmd.subscribe((data) => {
      helper.log('Failed to download package docs:\n' + data);
    });
    this.indexer.ports.dependenciesNotFoundCmd.subscribe(([projectDirectories, notFoundDependencies]) => {
      ...
      projectDirectories.forEach((projectDirectory) => {
        notFoundDependencies.forEach(([packageName, version]) => {
          const packageDirectory = path.resolve(projectDirectory, 'elm-stuff', 'packages', packageName, version);
          const elmPackageJsonFilePath = path.resolve(packageDirectory, 'elm-package.json');
          if (fs.existsSync(elmPackageJsonFilePath)) {
            let elmPackageJson = null;
            try {
              elmPackageJson = JSON.parse(fs.readFileSync(elmPackageJsonFilePath).toString());
            } catch(e) {
            }
            ...
            helper.log('Parsing elm-stuff package directory ' + packageDirectory + '...');
            sourceDirectories.forEach((sourceDirectory) => {
              indexing.parseDirectoryFiles(this.indexer, projectDirectory, path.resolve(packageDirectory, sourceDirectory));
            });
            helper.log('Parsed elm-stuff package directory ' + packageDirectory, 'green');
          }
        });
      });
    });
