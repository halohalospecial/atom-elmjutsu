      ...
      if (helper.isElmEditor(editor)) {
        ...
        this.subscriptions.add(editor.onDidChangeCursorPosition(({cursor}) => {
          ...
          this.sendTokenDebouncer =
            setTimeout(() => {
              // Only send active token or hole if nothing is selected.
              let selectedRange = editor.getSelectedBufferRange();
              if (selectedRange.isEmpty()) {
                const hole = this.inferTypes.getInferenceAtCursorPosition(editor);
                if (hole) {
                  indexing.sendEnteredHole(this.indexer, hole);
                } else {
