...
export default class Core {
  constructor() {
    ...
    //         indexer.ports.configChangedSub.send(data);
    ...
    //
    //       case 'docsRead':
    //         indexer.ports.docsReadSub.send(data);
    //         break;
    //
    //       case 'downloadMissingPackageDocs':
    //         indexer.ports.downloadMissingPackageDocsSub.send(data);
    //         break;
    //
    //       case 'configChanged':
    //         indexer.ports.configChangedSub.send(data);
    ...
    //         indexer.ports.fileContentsChangedSub.send(data);
    //         break;
    //
    //       case 'fileContentsRemoved':
    //         indexer.ports.fileContentsRemovedSub.send(data);
    //         break;
    //
    //       case 'activeFileChanged':
    //         indexer.ports.activeFileChangedSub.send(data);
    //         break;
    //
    //       case 'clearLocalHintsCache':
    //         indexer.ports.clearLocalHintsCacheSub.send(data);
    //         break;
    //
    //       case 'activeTokenChanged':
    //         indexer.ports.activeTokenChangedSub.send(data);
    //         break;
    //
    //       case 'inferenceEntered':
    //         indexer.ports.inferenceEnteredSub.send(data);
    //         break;
    //
    //       case 'projectDependenciesChanged':
    //         indexer.ports.projectDependenciesChangedSub.send(data);
    //         break;
    //
    //       case 'addImport':
    //         indexer.ports.addImportSub.send(data);
    //         break;
    //
    //       case 'showAddImportView':
    //         indexer.ports.showAddImportViewSub.send(data);
    //         break;
    //
    //       case 'getSuggestionsForImport':
    //         indexer.ports.getSuggestionsForImportSub.send(data);
    //         break;
    //
    //       case 'getHintsForPartial':
    //         indexer.ports.getHintsForPartialSub.send(data);
    //         break;
    //
    //       case 'constructFromTypeAnnotation':
    //         indexer.ports.constructFromTypeAnnotationSub.send(data);
    //         break;
    //
    //       case 'constructCaseOf':
    //         indexer.ports.constructCaseOfSub.send(data);
    //         break;
    //
    //       case 'constructDefaultArguments':
    //         indexer.ports.constructDefaultArgumentsSub.send(data);
    //         break;
    //
    //       case 'constructDefaultValueForType':
    //         indexer.ports.constructDefaultValueForTypeSub.send(data);
    //         break;
    //
    //       case 'getAliasesOfType':
    //         indexer.ports.getAliasesOfTypeSub.send(data);
    //         break;
    //
    //       case 'goToDefinition':
    //         indexer.ports.goToDefinitionSub.send(data);
    //         break;
    //
    //       case 'showGoToSymbolView':
    //         indexer.ports.showGoToSymbolViewSub.send(data);
    //         break;
    //
    //       case 'askCanGoToDefinition':
    //         indexer.ports.askCanGoToDefinitionSub.send(data);
    //         break;
    //
    //       case 'getImportersForToken':
    //         indexer.ports.getImportersForTokenSub.send(data);
    //         break;
    //     }
    //   };
    // });
            ...
            if (fs.existsSync(elmPackageJsonFilePath)) {
              ...
              sourceDirectories.forEach(sourceDirectory => {
                indexing.parseDirectoryFiles(
                  this.indexer,
                  projectDirectory,
                  path.resolve(packageDirectory, sourceDirectory)
                );
              });
              helper.log(
    ...
    this.indexer.onReadPackageDocs(missingDependencies => {
      ...
      this.indexer.postMessage({
        type: 'docsRead',
        data: readDependencyJsonStrings,
      });
      if (unreadDependencies.length > 0) {
        ...
        this.indexer.postMessage({
          type: 'downloadMissingPackageDocs',
          data: unreadDependencies,
        });
      }
    });
  ...
  watchProject(indexer, editor) {
    ...
    sourceDirectories.forEach(sourceDirectory => {
      indexing.parseDirectoryFiles(
        indexer,
        projectDirectory,
        path.resolve(projectDirectory, sourceDirectory)
      );
    });
