...
export default class HintTooltip {
  constructor(indexer, config) {
    ...
    indexer.onActiveTokenHintsChanged(activeTokenHints => {
      this.destroyTooltip();
      const editor = atom.workspace.getActiveTextEditor();
      if (atom.config.get('elmjutsu.showTypesInTooltip')) {
        if (this.activeFile && helper.isElmEditor(editor)) {
          const text = activeTokenHints
            .map(hint => {
              const moduleName =
                hint.moduleName === '' ||
                this.activeFile.filePath === hint.sourcePath
                  ? ''
                  : hint.moduleName + '.';
              const name =
                hint.name === helper.holeToken()
                  ? hint.name
                  : helper.isInfix(hint.name)
                    ? '(' + hint.name + ')'
                    : hint.name;
              const tipe =
                hint.tipe === ''
                  ? hint.args.length > 0
                    ? '<i>' + hint.args.join(' ') + '</i>'
                    : ''
                  : name !== '' ? ': ' + hint.tipe : hint.tipe;
              const aliasesOfTipe = atom.config.get(
                'elmjutsu.showAliasesOfTypesInTooltip'
              )
                ? hint.tipeAliases
                    .map(tipeAlias => {
                      return ' <i>a.k.a.</i> ' + tipeAlias;
                    })
                    .join('')
                : '';
              return (
                moduleName +
                (name.trim().length > 0 ? '<b>' + name + '</b> ' : '') +
                tipe +
                aliasesOfTipe
              );
            })
          ...
          if (text.trim().length > 0) {
            ...
            if (targetElement) {
              this.tooltip = atom.tooltips.add(targetElement, {
                ...
                trigger: 'manual',
                placement:
                  'auto ' + atom.config.get('elmjutsu.typesTooltipPosition'),
