...
export default class ElmMakeRunner {
  constructor(quickFix) {
    ...
    this.setMainPathsView.onDidConfirm(({ projectDirectory, mainPaths }) => {
      ...
      if (atom.config.get('elmjutsu.runElmMake') === 'on the fly') {
        const editor = atom.workspace.getActiveTextEditor();
        if (helper.isElmEditor(editor)) {
          this.forceCompile(editor);
        }
      }
    });
  }
  ...
  compile(editor) {
    ...
    if (alwaysCompileMain) {
      ...
      if (mainFilePaths) {
        if (isCompilingOnTheFly) {
          const tempFilesOfMainPaths = tempFiles.filter(tempFilePath => {
            return mainFilePaths.includes(
              this.tempToActualFileMapping[tempFilePath]
            );
          });
          if (tempFilesOfMainPaths.length > 0) {
            filePathsToCompile = tempFilesOfMainPaths;
          } else {
            return [];
          }
        } else {
      ...
      if (isCompilingOnTheFly && tempFiles.length > 0) {
        filePathsToCompile = tempFiles;
      }
    }
