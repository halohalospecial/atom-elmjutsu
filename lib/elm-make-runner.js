...
export default class ElmMakeRunner {
  ...
  compile(editor) {
    ...
    const elmExecPath = atom.config.get('elmjutsu.elmExecPath').trim();
    const elmTestExecPath = atom.config.get('elmjutsu.elmTestExecPath').trim();
    ...
    if (isTestFilePath) {
      args.unshift('--compiler=' + elmExecPath);
    }
    // const cwd = path.dirname(editorFilePath);
    const configExecutablePath = isTestFilePath ? elmTestExecPath : elmExecPath;
    const relativePath = path.join(projectDirectory, configExecutablePath);

    let executablePath;
    if (helper.fileExists(relativePath)) {
      executablePath = relativePath;
    } else if (helper.fileExists(configExecutablePath)) {
