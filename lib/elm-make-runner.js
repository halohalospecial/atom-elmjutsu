...
export default class ElmMakeRunner {
  ...
  compile(editor) {
    ...
    return atomLinter
      .exec(atom.config.get('elmjutsu.elmExecPath'), args, {
        ...
        // cwd: path.dirname(editorFilePath),
        // env: process.env,
      })
      .then(data => {
            ...
            const issues = _.flatten(
              result.errors.map(error => {
                return error.problems.map(problem => {
                  ...
                  return formatIssue(
                    this.toProblemFilePath(error.path),
                    problem.message,
                    range
                  );
                });
              ...
              _.flatten(
                result.errors.map(error => {
                  return error.problems.map(problem => {
                    ...
                    return {
                      filePath: this.toProblemFilePath(error.path),
  ...
  toProblemFilePath(filePath) {
    return this.tempToActualFileMapping[filePath] || filePath;
  }
