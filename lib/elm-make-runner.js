...
export default class ElmMakeRunner {
        ...
        .then(data => {
          ...
          if (data.stderr.length > 0) {
            ...
            if (result.type === 'compile-errors') {
              ...
              self.quickFix.computeFixes(projectDirectory, [
                {
          ...
          if (atom.config.get('elmjutsu.reportWarnings') == true) {
            ...
            helper.doInTempEditor(tempEditor => {
              ...
              // Add dummy type annotations.
              valuesWithoutTypeAnnotations.forEach(({ name, args }) => {
                tempEditor.backwardsScanInBufferRange(
                  new RegExp('^' + _.escapeRegExp(name) + ' '),
                  [[0, 0], editor.getEofBufferPosition()],
                  ({ match, stop, replace }) => {
                    if (match) {
                      const argAnnotations = args
                        .map(arg => {
                          return inferTypesHelper.dummyType();
                        })
                        .join(' -> ');
                      replace(
                        name +
            ...
            const names = valuesWithoutTypeAnnotations.map(({ name }) => {
              return name;
            });
  ...
  getInferredTypeAnnotations(
    ...
    atomLinter.tempFile(path.basename(editorFilePath), code, tempFilePath => {
      ...
      const uniqueKey = 'inferTypeAnnotations: ' + editorFilePath;
      return atomLinter
        .exec(executablePath, args, {
          uniqueKey,
          stream: 'both', // stdout and stderr
        ...
        .then(data => {
          ...
          problems.forEach(problem => {
            const match = problem.message[0].match(
              /^Something is off with the body of the `(.+)` definition:/
            );
            if (match && match.length > 1) {
              const nameInProblem = match[1];
              if (names.includes(nameInProblem)) {
                const inferredType = inferTypesHelper.getInferredTypeFromProblem(
                  problem
                );
                if (inferredType) {
                  console.log(nameInProblem + ' : ' + inferredType); // // //
                }
              }
            }
          });
