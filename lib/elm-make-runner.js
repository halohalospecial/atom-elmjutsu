...
export default class ElmMakeRunner {
  ...
  getInferredTypeAnnotations(names, projectDirectory, editor, resolve) {
    ...
    const proc = evalHelper.executeElmRepl(
      ...
      (errString, outString) => {
        if (errString.length === 0) {
          ...
          const issues = _
            .zip(names, outString.split('\n'))
            .map(([name, result]) => {
              const typeAnnotation = result.split(' : ')[1];
              const message = [
                'Top-level value `' +
                  name +
              ...
              let issueRange = new Range([0, 0], [0, 1]);
              ...
              let issue = formatIssue(filePath, message, issueRange, 'warning');
    ...
    const match = indexing.parseModuleNameAndComment(text);
    if (match) {
      maybeImportModule = 'import ' + match.name + ' exposing (..)';
    }
    evalHelper.sendToRepl(proc, helper.defaultImports(elmVersion), elmVersion);
    evalHelper.sendToRepl(proc, imports + '\n' + maybeImportModule, elmVersion);
    names.forEach(name => {
      evalHelper.sendToRepl(proc, name + '\n', elmVersion);
    });
  }
