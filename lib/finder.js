...
import FindUnusedView from './find-unused-view';
...
export default class Finder {
  ...
  findUsages() {
    ...
    let moduleName = null;
    // Get module name.
    editor.scanInBufferRange(/(?:^|\n)((effect|port)\s+)?(module)\s+(\S+)(\s|$)/, [[0, 0], editor.getEofBufferPosition()], ({match, stop}) => {
      moduleName = match[4];
      stop();
    });
    if (!moduleName) {
    ...
    helper.getProjectBuildArtifactsDirectory(editorFilePath).then((buildArtifactsDirectory) => {
  ...
  viewElmFile(projectDirectory, rangeText, moduleName, functionName, index, isPreview) {
    ...
    return atom.workspace.open(filePath, {pending: isPreview}).then((editor) => {
      ...
      editor.scanInBufferRange(helper.symbolDefinitionRegex(functionName), [[0, 0], editor.getEofBufferPosition()], ({match, range, stop}) => {
        // const diff = match[1].length - match[0].length;
        // usageRange = range.translate([0, 0], [0, diff]);
        const symbolMatch = match[2];
        usageRange = new Range(range.start, range.start.translate([0, symbolMatch.length]));
        stop();
      });
      if (rangeText) {
      ...
      if (isPreview) {
        ...
        editor.decorateMarker(self.usageMarker, {type: 'highlight', class: 'elmjutsu-usage-highlight'});
      }
