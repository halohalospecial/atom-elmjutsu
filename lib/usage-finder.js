...
export default {
  findUsagesOfSymbolAtCursor(indexer, activeEditor, indexDelta, callback) {
    ...
    const importersForTokenReceived = indexer.onImportersForTokenReceived(
      (
        [
          projectDirectory,
          rawToken,
          ...
          sourcePathAndNamesList,
        ]
      ) => {
        importersForTokenReceived.dispose();
        var allLineUsages = [];
        const tokenParts = rawToken.split('.');
        const tokenLastPart = tokenParts.pop();
        const tokenFirstParts = tokenParts.join('.');
        const tokenToHighlight = willUseFullToken
          ? rawToken
          : isCursorAtLastPartOfToken ? tokenLastPart : tokenFirstParts;
        sourcePathAndNamesList.forEach(
          ([sourcePath, isModule, isImportAlias, names]) => {
        ...
        const usages = _.uniq(_.flatten(allLineUsages, true), false, item => {
        ...
        const selectedIndex =
          ...
          Math.max(
            0,
            usages.findIndex(({ sourcePath, range }) => {
    ...
    indexer.getImportersForToken([
      helper.getProjectDirectory(filePath),
      helper.getToken(activeEditor),
      helper.isCursorAtLastPartOfToken(activeEditor),
    ]);
  },
};
...
function parseLine(
  ...
  const localNamesRegex = names
    .map(name => {
      return _.escapeRegExp(name);
    })
    .join('|');
  const moduleSymbolRegex = isModule
    ? `|${tokenToHighlight}\\.((?!${boundaryRegex}).)+`
    : '';
  ...
  while (match) {
    ...
    const trimmedMatchText = matchText.replace(
      new RegExp(tokenLastPart + '$'),
      ''
    );
    const tokenSubTokenDiff =
      !willUseFullToken && !isImportAlias && isCursorAtLastPartOfToken
        ? trimmedMatchText.length
        : 0;
    ...
    usages.push({
      ...
      range: new Range(
        new Point(row, index),
        new Point(row, index + tokenToHighlight.length)
      ),
    });
    match = regex.exec(line);
  }
