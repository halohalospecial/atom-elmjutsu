...
export default {
  ...
  activate() {
    ...
    startTime = helper.getTimestamp();
    this.quickFix = new QuickFix();
    helper.debugLog(
      'QuickFix activated ' + (helper.getTimestamp() - startTime) + ' ms'
    );
    startTime = helper.getTimestamp();
    this.elmMake = new ElmMake(this.quickFix);
    helper.debugLog(
      'ElmMake activated ' + (helper.getTimestamp() - startTime) + ' ms'
    );
    startTime = helper.getTimestamp();
  ...
  provideLinter() {
    ...
    let linter = {
      ...
      lintOnFly: false,
      lint(editor) {
        return self.elmMake.compile(editor);
      },
    };
    this.subscriptions.add(
