...
import {CompositeDisposable, Task, Emitter} from 'atom';
...
export default {
  ...
  activate() {
    ...
    this.sidekick = new Sidekick(this.core.getIndexer(), this.storeJumpPointFunction);
    this.hintTooltip = new HintTooltip(this.core.getIndexer());
    this.datatip = new Datatip(this.core.getIndexer(), this.storeJumpPointFunction);
  ...
  provideAutocomplete() {
    ...
    const getInferenceAtCursorPosition = (editor) => {
      return this.inferTypes && this.inferTypes.getInferenceAtCursorPosition(editor);
    };
  ...
  consumeDatatipService(datatipService) {
    const getInferenceAtPosition = (editor, position) => {
      return this.inferTypes && this.inferTypes.getInferenceAtPosition(editor, position);
    };
