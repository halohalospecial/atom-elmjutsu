...
export default class AddImport {
  constructor(indexer) {
    ...
    this.indexer.onShowAddImportView(
      ([defaultSymbolName, alias, activeFile, symbols]) => {
        this.view.show();
        this.view.setSymbols(
          defaultSymbolName,
          alias,
          activeFile.filePath,
  ...
  addImportCommand() {
    ...
    if (helper.isElmEditor(editor)) {
      this.indexer.showAddImportView([
        editor.getPath(),
  ...
  addImportAsCommand(token) {
    const editor = atom.workspace.getActiveTextEditor();
    if (helper.isElmEditor(editor)) {
      this.indexer.showAddImportView([
        editor.getPath(),
        token || helper.getToken(editor),
        true,
      ]);
    }
  }
