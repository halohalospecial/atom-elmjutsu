      ...
      if (atom.config.get('elmjutsu.hyperclickEnabled') && helper.isElmEditor(editor)) {
        return new Promise((resolve) => {
          ...
          if (!helper.isScopeAString(scopeDescriptor) && !helper.isScopeAComment(scopeDescriptor)) {
              ...
              if (yes) {
                return resolve({
                  ...
                  callback() {
                    ...
                    indexer.ports.goToDefinitionSub.send([token, helper.getActiveTopLevel(editor, position)]);
                  }
                });
              } else {
                return resolve();
            ...
            indexer.ports.askCanGoToDefinitionSub.send([token, helper.getActiveTopLevel(editor, position)]);
          } else {
            return resolve();
          }
        });
