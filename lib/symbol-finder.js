...
export default class SymbolFinder {
  ...
  reenableLinterPackage() {
    this.justDisabledLintingPackages.forEach(packageName => {
  ...
  storeStateBeforeFind(editor) {
    ...
    this.stateBeforeFind = {
      existingEditorIds: new Set(
        atom.workspace.getTextEditors().map(({ id }) => id)
      ),
  ...
  closePreviewEditors(exceptEditorId) {
    atom.workspace.getTextEditors().forEach(textEditor => {
      if (
        (!exceptEditorId ||
          (exceptEditorId && textEditor.id !== exceptEditorId)) &&
        !this.stateBeforeFind.existingEditorIds.has(textEditor.id)
      ) {
        textEditor.destroy();
      }
    });
  ...
  viewInEditor(symbol, isPreview) {
    return atom.workspace
      .open(symbol.sourcePath, { pending: isPreview })
      .then(editor => {
        ...
        if (isPreview) {
          this.destroyUsageMarker();
          this.symbolMarker = editor.markBufferRange(symbolRange, {
            invalidate: 'never',
            persistent: false,
          });
          editor.decorateMarker(this.symbolMarker, {
            type: 'highlight',
            class: 'elmjutsu-symbol-marker',
          });
        }
        if (!isPreview) {
  ...
  show(defaultSymbolFullName, activeFilePath, symbols) {
    // Temporarily disable `linter` package (if not alredy disabled).
    // This avoids linting the previewed file.
    this.linterPackageJustDisabled =
      !atom.packages.isPackageDisabled('linter') &&
      atom.packages.disablePackage('linter');
